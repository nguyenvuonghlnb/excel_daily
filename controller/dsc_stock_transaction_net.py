...
def data_xlsm_stock_transaction():
    ...
    if read_not:
        ...
        if date_str == today.strftime("%Y-%m-%d"):
            ...
            volume_buy, value_buy, volume_sell, value_sell, volume_net, value_net, data_collection_date) VALUES (%s, 
            %s, %s, %s, %s, %s, %s, %s, %s, %s)ON CONFLICT ON CONSTRAINT dsc_mst_asset_transaction_statistics_pkey DO 
            UPDATE SET (category, date, stock_code,volume_buy, value_buy, volume_sell, value_sell, volume_net, 
            value_net, data_collection_date) = ( EXCLUDED.category, EXCLUDED.date, EXCLUDED.stock_code, 
            EXCLUDED.volume_buy, EXCLUDED.value_buy, EXCLUDED.volume_sell, EXCLUDED.value_sell, EXCLUDED.volume_net, 
            EXCLUDED.value_net, EXCLUDED.data_collection_date)''',
                            [[
                                item.get("category"),
                                item.get("date"),
                                item.get("stock_code"),
